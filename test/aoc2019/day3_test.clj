...
;; testing data
(def wire-str1 "R8,U5,L5,D3")
(def wire-str2 "U7,R6,D4,L4")
(def sd-12 6)
...
(def sd-full 3247)
(def wire-steps-full 48054)
;; wire-min-steps: 48054

;; Day 3 - part 1
;; shortest distance of intersecting wires to origin
;;

(deftest convert-direction-test
  (testing "R8 -> "
    (is (= (convert-direction "R8") {:right 8})))
  (testing "L3 -> "
    (is (= (convert-direction "L3") {:left 3})))
  (testing "D2 -> "
    (is (= (convert-direction "D2") {:down 2})))
  (testing "U98 -> "
    (is (= (convert-direction "U98") {:up 98})))
  )
  
(deftest parse-wire-test
  (testing "one direction"
    (is (= (parse-wire "R8") [{:right 8}])))
  (testing "two directions"
    (is (= (parse-wire "R8,D5") [{:right 8} {:down 5}])))
  (testing "three directions"
    (is (= (parse-wire "R8,D5,L2") [{:right 8} {:down 5}{:left 2}])))
  )

(deftest compute-new-pos-test
  (testing "right 90 from 10,10"
    (is (= (compute-new-pos 10 10 :right 90) {:x 100 :y 10})))
  (testing "left 90 from 100,10"
    (is (= (compute-new-pos 100 10 :left 90) {:x 10 :y 10})))
  (testing "up 90 from 10,10"
    (is (= (compute-new-pos 10 10 :up 90) {:x 10 :y 100})))
  (testing "down 90 from 10,100"
    (is (= (compute-new-pos 10 100 :down 90) {:x 10 :y 10})))
  )

(deftest next-point-test
  (testing "Starting with 0,0 and adding :right 90"
    (is (= (next-point [{:x 0 :y 0}] {:right 90}) [{:x 0 :y 0}{:x 90 :y 0}])))
  (testing "Starting with 0,0 and adding :up 10"
    (is (= (next-point [{:x 0 :y 0}] {:up 10}) [{:x 0 :y 0}{:x 0 :y 10}])))
  (testing "Starting with 0,0 and adding :left 90"
    (is (= (next-point [{:x 0 :y 0}] {:left 90}) [{:x 0 :y 0}{:x -90 :y 0}])))
  (testing "Starting with 0,0 and adding :down 90"
    (is (= (next-point [{:x 0 :y 0}] {:down 90}) [{:x 0 :y 0}{:x 0 :y -90}])))
  )

(deftest cons-wire-test
  (testing "construct a wire data set for R8 -> 2 points"
    (is (= (cons-wire "R8") [{:x 0 :y 0}
                             {:x 8 :y 0}])))
  (testing "construct a wire data set for R8,U5 -> 3 points"
    (is (= (cons-wire "R8,U5") [{:x 0 :y 0}
                                {:x 8 :y 0}
                                {:x 8 :y 5}])))
  (testing "construct a wire data set -> 5 points"
    (is (= (cons-wire "R8,U5,L5,D3") [{:x 0 :y 0}
                                      {:x 8 :y 0}
                                      {:x 8 :y 5}
                                      {:x 3 :y 5}
                                      {:x 3 :y 2} ])))
  (testing "construct a wire data set for 5 points with negative offsets"
    (is (= (cons-wire "R3,U3,L10,D20") [{:x 0 :y 0}
                                        {:x 3 :y 0}
                                        {:x 3 :y 3}
                                        {:x -7 :y 3}
                                        {:x -7 :y -17} ])))
...
(deftest shortest-distance-test
  (testing "example R8,U5,L5,D3 x U7,R6,D4,L4 -> 6"
    (is (= (shortest-distance "R8,U5,L5,D3" "U7,R6,D4,L4") 6)))
  (testing "1. test: R75,D30,R83,U83,L12,D49,R71,U7,L72 x U62,R66,U55,R34,D71,R55,D58,R83 -> 159"
    (is (= (shortest-distance "R75,D30,R83,U83,L12,D49,R71,U7,L72" "U62,R66,U55,R34,D71,R55,D58,R83") 159)))
  (testing "2. test: R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51 x U98,R91,D20,R16,D67,R40,U7,R15,U6,R7 -> 135"
    (is (= (shortest-distance "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51" "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7") 135)))
  (testing "full input from aoc"
    (is (= sd-full (shortest-distance wire-full-1 wire-full-2))))
  )

(deftest comp-steps-test
  (testing "wire 1, point 6 5 -> 15"
    (is (= (comp-steps 0 (cons-wire-parts "R8,U5,L5,D3") (Point. 6 5)) 15)))
  (testing "wire 2, point 6 5 -> 15"
    (is (= (comp-steps 0 (cons-wire-parts "U7,R6,D4,L4") (Point. 6 5)) 15)))
  (testing "wire 1, point 3 3 -> 20"
    (is (= (comp-steps 0 (cons-wire-parts "R8,U5,L5,D3") (Point. 3 3)) 20)))
  (testing "wire 2, point 3 3 -> 20"
    (is (= (comp-steps 0 (cons-wire-parts "U7,R6,D4,L4") (Point. 3 3)) 20)))
  )

(deftest wire-min-steps-test
  (testing "example R8,U5,L5,D3 x U7,R6,D4,L4 -> 30 steps"
    (is (= 30 (wire-min-steps "R8,U5,L5,D3" "U7,R6,D4,L4"))))
  (testing "610 steps"
    (is (= 610 (wire-min-steps "R75,D30,R83,U83,L12,D49,R71,U7,L72" "U62,R66,U55,R34,D71,R55,D58,R83"))))
  (testing "410 steps"
    (is (= 410 (wire-min-steps "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"))))
  (testing "full input from aoc"
    (is (= wire-steps-full (wire-min-steps wire-full-1 wire-full-2))))
  )
